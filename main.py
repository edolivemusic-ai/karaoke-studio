...
# Fix critico per PyInstaller --windowed (stdout/stderr)
...
def _get_whisper_model_dir():
    """Preferisce i modelli bundled con l'EXE"""
...
def parse_time(s):
    try:
        ...
        if ":" in s:
            ...
            if len(parts) == 2:
                m, rest = parts
                sv, ms = rest.split(".") if "." in rest else (rest, "0")
...
# Gradient presets e posizioni QR
GRADIENTS = {
...
QR_POS_MAP = {
    ...
    "Alto sinistra": ("20", "20"),
    "Centro destra": ("W-{s}-20", "(H-{s})/2"),
    "Centro sinistra": ("20", "(H-{s})/2"),
}
...
class KaraokeApp(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        ctk.CTkLabel(hdr, text="🎤  KaraokeAI Studio",
                     font=ctk.CTkFont("Courier New", 22, "bold"), text_color=ACCENT2).pack(side="left", padx=24)
        ctk.CTkLabel(hdr, text="trascrivi  •  sincronizza  •  esporta",
                     font=ctk.CTkFont("Courier New", 11), text_color=MUTED).pack(side="left", padx=4)
    ...
    def _section(self, parent, title):
        ...
        ctk.CTkLabel(outer, text=title, font=ctk.CTkFont(size=13, weight="bold"),
                     text_color=ACCENT2).pack(anchor="w", pady=(0, 4))
        return outer
    ...
    def _left_panel(self, parent):
        ...
        self.audio_label = ctk.CTkLabel(s1, text="Nessun file caricato",
                                        text_color=MUTED, font=ctk.CTkFont(size=12), wraplength=270)
        self.audio_label.pack(pady=(0, 6))
        ctk.CTkButton(s1, text="📂  Scegli file audio", command=self._load_audio,
                      fg_color=ACCENT, hover_color=ACCENT2, height=36, corner_radius=8).pack(fill="x")

        # Step 2: Lingua
        s2 = self._section(frame, "② Lingua (opzionale)")
        self.lang_var = ctk.StringVar(value="Auto")
        ctk.CTkOptionMenu(s2, variable=self.lang_var,
                          values=["Auto", "Italiano", "English", "Español", "Français", "Deutsch",
                                  "Português", "日本語", "한국어", "中文", "Русский", "العربية"],
        ...
        self.sync_btn = ctk.CTkButton(s4, text="🎙  Avvia Trascrizione AI",
                                      command=self._start_sync, fg_color=ACCENT,
                                      hover_color=ACCENT2, height=44, corner_radius=10,
        ...
        # Gradient
        rg = ctk.CTkFrame(s5, fg_color="transparent")
        ...
        ctk.CTkOptionMenu(rg, variable=self.grad_var, values=list(GRADIENTS.keys()),
                          ...
                          dropdown_fg_color=CARD, width=170).pack(side="left", padx=6)

        # Font size
        rf = ctk.CTkFrame(s5, fg_color="transparent")
        rf.pack(fill="x", pady=(0, 4))
        ctk.CTkLabel(rf, text="Font:", text_color=MUTED, width=80).pack(side="left")
        self.font_size = ctk.CTkSlider(rf, from_=32, to=96, number_of_steps=16,
                                       fg_color=SURFACE, progress_color=ACCENT)
        self.font_size.set(64)
        self.font_size.pack(side="left", fill="x", expand=True, padx=6)

        # Text color
        rc = ctk.CTkFrame(s5, fg_color="transparent")
        rc.pack(fill="x", pady=(0, 4))
        ctk.CTkLabel(rc, text="Colore:", text_color=MUTED, width=80).pack(side="left")
        self.txt_color_var = ctk.StringVar(value="bianco")
        ctk.CTkOptionMenu(rc, variable=self.txt_color_var,
                          values=["bianco", "giallo", "ciano", "rosa"],
                          fg_color=SURFACE, button_color=ACCENT,
                          dropdown_fg_color=CARD, width=170).pack(side="left", padx=6)

        # QR Code
        sq = self._section(frame, "🖼  Logo / Immagine pub (opzionale)")
        self.qr_label = ctk.CTkLabel(sq, text="Nessuna immagine caricata",
        ...
        # QR Position & Size
        rqp = ctk.CTkFrame(sq, fg_color="transparent")
        rqp.pack(fill="x", pady=(0, 4))
        ...
        rqs = ctk.CTkFrame(sq, fg_color="transparent")
        rqs.pack(fill="x", pady=(0, 8))
        ctk.CTkLabel(rqs, text="Dimensione:", text_color=MUTED, width=80).pack(side="left")
        self.qr_size = ctk.CTkSlider(rqs, from_=80, to=400, number_of_steps=32,
                                     fg_color=SURFACE, progress_color=ACCENT)
        self.qr_size.set(160)
        self.qr_size.pack(side="left", fill="x", expand=True, padx=6)
        self.qr_size_lbl = ctk.CTkLabel(rqs, text="160px", text_color=MUTED, width=48,
                                        font=ctk.CTkFont(size=11))
        self.qr_size_lbl.pack(side="left")
        self.qr_size.configure(command=lambda v: self.qr_size_lbl.configure(text=f"{int(v)}px"))

        self.export_btn = ctk.CTkButton(sq, text="⬇  Esporta MP4 1080p",
                                        command=self._export_video, fg_color=SUCCESS,
                                        hover_color="#16a34a", height=40, corner_radius=10,
    ...
    def _right_panel(self, parent):
        ...
        ctk.CTkLabel(hdr, text="✏️  Editor Sincronizzazione",
                     font=ctk.CTkFont(size=15, weight="bold"), text_color=TEXT).pack(side="left")
        ctk.CTkButton(hdr, text="🗑 Tutto", width=80, height=28,
                      command=self._clear_rows, fg_color=SURFACE,
                      hover_color=DANGER, corner_radius=6).pack(side="right", padx=6)
        ctk.CTkButton(hdr, text="+ Riga", width=80, height=28,
                      command=self._add_row, fg_color=SURFACE,
                      hover_color=ACCENT, corner_radius=6).pack(side="right")

        # Headers
        cols = ctk.CTkFrame(frame, fg_color=SURFACE, corner_radius=6)
        cols.pack(fill="x", padx=16, pady=(0, 4))
        for txt, w in [("#", 36), ("Testo trascritto", 0), ("Inizio", 88), ("Fine", 88), ("", 40)]:
            ctk.CTkLabel(cols, text=txt, text_color=MUTED,
                         font=ctk.CTkFont(size=11), width=w or 1).pack(side="left", padx=6, pady=4)
        ...
        prev_hdr.pack(fill="x", padx=16, pady=(0, 4))
        prev_hdr.pack_propagate(False)
        ctk.CTkLabel(prev_hdr, text="🖥  Anteprima",
                     font=ctk.CTkFont(size=12, weight="bold"), text_color=ACCENT2).pack(side="left", padx=12, pady=5)

        self.preview = tk.Canvas(frame, bg="#08080f", height=130,
    ...
    def _make_row(self, i, line):
        ...
        ctk.CTkLabel(row, text=str(i + 1), width=32, text_color=MUTED,
                     font=ctk.CTkFont(size=11)).pack(side="left", padx=4)

        txt_e = ctk.CTkEntry(row, font=ctk.CTkFont("Courier New", 12),
                             fg_color="transparent", text_color=TEXT, border_width=0)
        txt_e.insert(0, line["text"])
        txt_e.pack(side="left", fill="x", expand=True, padx=4)
        txt_e.bind("<FocusOut>", lambda e, idx=i, en=txt_e: self._update_text(idx, en.get()))

        # Start / End
        for key, color in [("start", SUCCESS), ("end", WARN)]:
            e = ctk.CTkEntry(row, width=84, font=ctk.CTkFont("Courier New", 11),
                             fg_color="#0d0d0f", text_color=color, border_color=SURFACE)
            e.insert(0, fmt_time(line[key]))
            e.pack(side="left", padx=4)
            e.bind("<FocusOut>", lambda e, idx=i, k=key, en=e: self._update_time(idx, k, en.get()))

        ctk.CTkButton(row, text="✕", width=32, height=24, fg_color="transparent",
                      hover_color=DANGER, text_color=MUTED, corner_radius=4,
    ...
    def _update_time(self, idx, key, val):
        if 0 <= idx < len(self.lyrics_lines):
            self.lyrics_lines[idx][key] = parse_time(val)

    def _delete_row(self, idx):
        if 0 <= idx < len(self.lyrics_lines):
            self.lyrics_lines.pop(idx)
            self._render_rows()

    def _add_row(self):
        last = self.lyrics_lines[-1]["end"] if self.lyrics_lines else 0.0
        self.lyrics_lines.append({"text": "Nuovo verso", "start": last, "end": last + 3.0})
        self._render_rows()
    ...
    def _load_audio(self):
        ...
        try:
            r = subprocess.run([FFPROBE, "-v", "quiet", "-print_format", "json",
                                "-show_format", path], capture_output=True, text=True, timeout=15)
            info = json.loads(r.stdout)
            self.audio_duration = float(info["format"]["duration"])
    ...
    def _load_qr(self):
        path = filedialog.askopenfilename(
            title="Scegli immagine pubblicitaria",
    ...
    def _start_sync(self):
        if not self.audio_path:
            messagebox.showwarning("Attenzione", "Prima carica un file audio!")
            return
    ...
    def _sync_worker(self):
        try:
            ...
            if self.whisper_model is None or getattr(self.whisper_model, "_name", "") != model_name:
                self.after(0, lambda: self.status_lbl.configure(text="Caricamento modello AI...", text_color=ACCENT))
                self.whisper_model = whisper.load_model(model_name, download_root=WHISPER_MODEL_DIR)
                self.whisper_model._name = model_name

            lang_map = {
                "Auto": None, "Italiano": "it", "English": "en", "Español": "es",
                "Français": "fr", "Deutsch": "de", "Português": "pt", "日本語": "ja",
                "한국어": "ko", "中文": "zh", "Русский": "ru", "العربية": "ar"
            }
            language = lang_map.get(self.lang_var.get())
            ...
            self.after(0, lambda: self.status_lbl.configure(
                text=f"✅ {len(lines)} versi trascritti • lingua: {detected}",
                text_color=SUCCESS
            ))
            ...
            self.after(0, lambda: self.sync_btn.configure(
                state="normal", text="🎙  Avvia Trascrizione AI"
            ))
    ...
    def _draw_preview(self):
        ...
        stops = GRADIENTS.get(self.grad_var.get(), [(80, 0, 160), (0, 20, 180)])
        r1, g1, b1 = stops[0]
        r2, g2, b2 = stops[1]
        ...
        if not self.lyrics_lines:
            c.create_text(w//2, h//2, text="Trascrivi per vedere l'anteprima",
                          fill="white", font=("Courier New", 11))
            return
        ...
        c.create_text(w//2, h//2-10, text=curr.upper(), fill="white",
                      font=("Courier New", 15, "bold"), anchor="center")
        if nxt:
            c.create_text(w//2, h//2+26, text=nxt, fill="#cccccc",
                          font=("Courier New", 10), anchor="center")

    # ====================== EXPORT ======================
    def _export_video(self):
        if not self.audio_path:
            messagebox.showwarning("Attenzione", "Carica prima un file audio!")
            return
        if not self.lyrics_lines:
            messagebox.showwarning("Attenzione", "Trascrivi prima la canzone!")
            return

        out_path = filedialog.asksaveasfilename(
            title="Salva video karaoke",
        ...
        self.export_status.configure(text="Generazione video in corso...", text_color=MUTED)
    ...
    def _export_worker(self, out_path, grad_name, font_size, txt_color, qr_path, qr_size, qr_pos):
        ...
        try:
            ...
            color_map = {
                "bianco": "&H00FFFFFF", "giallo": "&H0000FFFF",
                "ciano": "&H00FFFF00", "rosa": "&H00FF80FF"
            }
            ass_color = color_map.get(txt_color, "&H00FFFFFF")

            ass_path = self._build_ass(font_size, ass_color)

            r1, g1, b1 = GRADIENTS.get(grad_name, [(80, 0, 160), (0, 20, 180)])[0]
            r2, g2, b2 = GRADIENTS.get(grad_name, [(80, 0, 160), (0, 20, 180)])[1]

            geq = f"geq=r='({r1}+({r2}-{r1})*Y/H)':g='({g1}+({g2}-{g1})*Y/H)':b='({b1}+({b2}-{b1})*Y/H)'"

            if qr_path and os.path.exists(qr_path):
                x_expr, y_expr = QR_POS_MAP.get(qr_pos, ("W-{s}-20", "H-{s}-20"))
                x = x_expr.replace("{s}", str(qr_size)).replace("W", "main_w").replace("H", "main_h")
                y = y_expr.replace("{s}", str(qr_size)).replace("W", "main_w").replace("H", "main_h")

                cmd = [
                    FFMPEG, "-y",
                    "-f", "lavfi", "-i", f"color=black:s=1920x1080:d={dur}",
                    "-i", self.audio_path,
                    "-i", qr_path,
                    "-filter_complex",
                    f"[0:v]{geq}[bg];[2:v]scale={qr_size}:{qr_size}[qr];"
                    f"[bg][qr]overlay={x}:{y}[bgqr];[bgqr]ass={ass_path}[v]",
                    "-map", "[v]", "-map", "1:a",
                ...
                cmd = [
                    ...
                    "-i", self.audio_path,
                    "-filter_complex",
                    f"[0:v]{geq}[bg];[bg]ass={ass_path}[v]",
                    "-map", "[v]", "-map", "1:a",
                    "-c:v", "libx264", "-preset", "medium", "-crf", "23",
                    "-c:a", "aac", "-b:a", "192k", "-shortest", out_path
                ]
