...
class KaraokeApp(ctk.CTk):
    ...
    def _export_worker(self,out_path,grad_name,font_size,txt_color,qr_path,qr_size,qr_pos):
        try:
            ...
            r1,g1,b1=stops[0]; r2,g2,b2=stops[1]
            # Filtro gradiente corretto (senza parentesi esterne nelle espressioni)
            ...
            # Coordinate QR assolute (più affidabili delle espressioni FFmpeg)
            W,H=1920,1080
            x_final=20 if "sinistra" in qr_pos.lower() else W-qr_size-20
            y_final=20 if "alto" in qr_pos.lower() else H-qr_size-20
            if "centro" in qr_pos.lower(): y_final=(H-qr_size)//2
            if qr_path and os.path.exists(qr_path):
                cmd=[FFMPEG,"-y",
                    "-f","lavfi","-i",f"color=black:s=1920x1080:r=30:d={dur}",
                    "-i",self.audio_path,"-i",qr_path,
                    "-filter_complex",
                    f"[0:v]geq={geq}[bg];"
                    f"[2:v]scale={qr_size}:{qr_size}[qr];"
                    f"[bg][qr]overlay={x_final}:{y_final}[bgqr];"
                    f"[bgqr]{ass_filter}[v]",
                    "-map","[v]","-map","1:a",
    ...
    def _build_ass(self,font_size,primary_color):
        import uuid
        safe_dir=Path(os.environ.get("TEMP") or os.environ.get("TMP") or os.environ.get("TMPDIR") or str(_get_base()))
